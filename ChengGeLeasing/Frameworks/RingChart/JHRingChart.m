...
- (void)configBaseData{
    ...
    _totolCount = 0;
    _itemsSpace =0;  //(M_PI * 2.0 * 10 / 360)/_valueDataArr.count ;
...
//        if (_fillColorArray.count<_valueDataArr.count) {
        ...
        color = [UIColor blackColor]; //k_COLOR_STOCK[i%k_COLOR_STOCK.count];
//        }else{
//            color = _fillColorArray[i];
//        }
        
        NSString *firstV = _valueDataArr[_tIndex];
        
        NSInteger objV = 0;
        
        if(!IsStringEmpty(obj) && ![obj isEqualToString:@"0"]) {
            objV = [obj integerValue];
        }
        if ([self.totalNum  isEqualToString:@"0"] && [firstV isEqualToString:@"1"])
        {
            return;
        }
        ...
        CGSize size = [[NSString stringWithFormat:@"%.02f%c",[obj floatValue] / _totolCount * 100,'%'] boundingRectWithSize:CGSizeMake(200, 100) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15*k_Width_Scale]} context:nil].size;
        ...
        if (midSpace<M_PI) {
            ...
            if(objV>0) {
                [self drawText:[NSString stringWithFormat:@"%.02f%c",[obj floatValue] / _totolCount * 100,'%'] andContext:contex atPoint:CGPointMake(secondP.x + 3, secondP.y - size.height / 2) WithColor:color andFontSize:15*k_Width_Scale];
            }
...
//            [self drawText:self.valueDataTitlesArr[i] andContext:contex atPoint:CGPointMake(secondP.x + 3, secondP.y - size.height / 2 - 10) WithColor:color andFontSize:10*k_Width_Scale];
             ...
             secondP =CGPointMake(endx.x - 20*k_Width_Scale, endx.y);
            if(objV>0) {
                [self drawText:[NSString stringWithFormat:@"%.02f%c",[obj floatValue] / _totolCount * 100,'%'] andContext:contex atPoint:CGPointMake(secondP.x - size.width - 3, secondP.y - size.height/2) WithColor:color andFontSize:15*k_Width_Scale];
            }
