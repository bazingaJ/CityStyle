...
@implementation CGLeftViewCell

- (void)setLeftViewModel:(CGTeamXiangmuModel *)model frame:(CGRect)frame {
    if(!model) return;
    
    //创建“背景层”
    UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(10, 0, frame.size.width-20, 120)];
    ...
    //创建“项目名称”
    UIButton *btnFunc = [[UIButton alloc] initWithFrame:CGRectMake(70, 10, frame.size.width-80, 25)];
    [btnFunc setTitle:model.name forState:UIControlStateNormal];
    [btnFunc setTitleColor:COLOR3 forState:UIControlStateNormal];
    [btnFunc.titleLabel setFont:[UIFont systemFontOfSize:18]];
    ...
    UILabel *lbMsg2 = [[UILabel alloc] initWithFrame:CGRectMake(70, 40, frame.size.width-80, 20)];
