...
@interface CGQuotationSheetView ()

@property (nonatomic, strong) NSMutableArray *dataArr;

@end

@implementation CGQuotationSheetView
...
- (UITableView *)tableView {
    if(!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 45, self.frame.size.width, self.frame.size.height-45) style:UITableViewStyleGrouped];
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        ...
        [self.dataArr addObject:@[@"元/年",@"1"]];
        [self.dataArr addObject:@[@"元/月",@"2"]];
        [self.dataArr addObject:@[@"元/平方米/天",@"3"]];
        [self.dataArr addObject:@[@"元/平方米/月",@"4"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellIdentity = @"CGQuotationSheetViewCell";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSArray *itemArr = [self.dataArr objectAtIndex:indexPath.row];
    if(self.callBack) {
        self.callBack(itemArr[1], itemArr[0]);
        [self dismiss];
    }
