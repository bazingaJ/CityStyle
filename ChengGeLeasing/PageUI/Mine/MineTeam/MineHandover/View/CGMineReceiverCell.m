...
@implementation CGMineReceiverCell
...
- (void)setTeamMemberModel:(CGTeamMemberModel *)model {
    ...
    [btnFunc addTouch:^{
        NSLog(@"选择");
        if([self.delegate respondsToSelector:@selector(CGMineReceiverCellClick:)]) {
            [self.delegate CGMineReceiverCellClick:model];
        }
    }];
