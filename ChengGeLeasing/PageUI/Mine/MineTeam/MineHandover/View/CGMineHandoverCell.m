...
@implementation CGMineHandoverCell
...
- (void)setTeamMemberModel:(CGTeamMemberModel *)model {
    ...
    [btnFunc addTouch:^{
        NSLog(@"选择");
        if([self.delegate respondsToSelector:@selector(CGMineHandoverCellClick:)]) {
            [self.delegate CGMineHandoverCellClick:model];
        }
    }];
