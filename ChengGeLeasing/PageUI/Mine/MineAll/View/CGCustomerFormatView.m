...
static NSString * const CGCustomerFormatViewCellID = @"CGCustomerFormatViewCellID";

@implementation CGCustomerFormatView
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[UICollectionViewCell class] forCellWithReuseIdentifier:CGCustomerFormatViewCellID];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UICollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:CGCustomerFormatViewCellID forIndexPath:indexPath];
    ...
    if([self.itemArr containsObject:model.id]) {
        //选中
        [btnFunc setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [btnFunc setBackgroundColor:MAIN_COLOR];
    ...
    [btnFunc addTouch:^{
        ...
        if(!self.isMultiple) {
            ...
            for (int i=0; i<self.dataArr.count; i++) {
                CGFormatModel *model = [self.dataArr objectAtIndex:i];
                model.is_selected = NO;
                [self.itemArr removeObject:model.id];
            }
        ...
        if(btnFunc.isSelected) {
            ...
            [btnFunc setBackgroundColor:MAIN_COLOR];
            model.is_selected = YES;
            [self.itemArr addObject:model.id];
        }else{
            [btnFunc setTitleColor:COLOR3 forState:UIControlStateNormal];
            [btnFunc setBackgroundColor:[UIColor whiteColor]];
            model.is_selected = NO;
