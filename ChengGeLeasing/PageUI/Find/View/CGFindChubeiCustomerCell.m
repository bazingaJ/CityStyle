...
#import "CGFindChubeiCustomerCell.h"

@implementation CGFindChubeiCustomerCell
...
- (void)setCustomerModel:(CGCustomerModel *)model indexPath:(NSIndexPath *)indexPath {
    ...
    [lbMsg setText:[NSString stringWithFormat:@"更新时间：%@",model.add_time]];
    ...
    if (!IsStringEmpty(model.logo)) {
        ...
        imgView.image = [UIImage avatarWithName:model.first_letter];
        [imgView setBackgroundColor:MAIN_COLOR];
    }
    ...
    [lbMsg3 setText:model.cate_name];
    ...
    //创建“数量”
    NSString *memNum = @"0";
    if(!IsStringEmpty(model.linkman_count)) {
        memNum = model.linkman_count;
    }
